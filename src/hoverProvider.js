...
// Cache for word ranges to avoid repeated calculations
const wordRangeCache = new Map();
...
function scanDocument(document) {
    ...
    // Update version and clear previous data
    ...
    try {
        for (let i = 0; i < document.lineCount; i++) {
            ...
            const declarationsMatch = line.match(DECLARATION_PATTERN);
            if (declarationsMatch && declarationsMatch[1]) {
                localFunctions.set(declarationsMatch[1], {
                    type: 'declaration',
                    content: line,
                    line: i
                });
                continue;  // Skip to next line after finding a declaration
            }

            // Check for reducible expressions
            const reducibleMatch = line.match(REDUCIBLE_PATTERN);
            if (reducibleMatch && reducibleMatch[1]) {
                const functionName = reducibleMatch[1];
                if (!localFunctions.has(functionName)) {
                    localFunctions.set(functionName, {
                        type: 'reducible',
                        content: 'Reducible expression',
                        line: i
                    });
                }
            }
...
function activate(context) {
    ...
    // Scan document when active editor changes
    context.subscriptions.push(
        vscode.window.onDidChangeActiveTextEditor(editor => {
            if (editor) {
                documentVersion = -1;  // Force rescan
                scanDocument(editor.document);
            }
        })
    );

    // Scan document when it changes
    context.subscriptions.push(
    ...
    context.subscriptions.push(
        vscode.languages.registerHoverProvider('metta', {
            provideHover(document, position, token) {
                ...
                // Rest of hover provider logic
                if (lastHoverPosition && 
                    position.line === lastHoverPosition.line && 
                    position.character === lastHoverPosition.character) {
                ...
                try {
                    ...
                    lastHoverResult = new vscode.Hover(hoverMessage);
...
// Cached position key generator
function getPositionKey(document, position) {
    return `${document.uri.toString()}-${position.line}-${position.character}`;
}

function getWordRangeAtPosition(document, position) {
    const positionKey = getPositionKey(document, position);

    // Check cache first
    if (wordRangeCache.has(positionKey)) {
        return wordRangeCache.get(positionKey);
    }

    // Manage cache size
    if (wordRangeCache.size >= MAX_CACHE_SIZE) {
        const firstKey = wordRangeCache.keys().next().value;
        wordRangeCache.delete(firstKey);
    }
    ...
    // Optimize range calculation by using string operations instead of multiple getText calls
    let startChar = position.character;
    while (startChar > 0 && WORD_PATTERN.test(lineText[startChar - 1])) {
        startChar--;
    }

    let endChar = position.character;
    while (endChar < lineText.length && WORD_PATTERN.test(lineText[endChar])) {
        endChar++;
    }

    // Only create range if word found
    ...
    const range = new vscode.Range(
        ...
        position.line, endChar
    );

    // Cache the result
    wordRangeCache.set(positionKey, range);

    return range;
}

// Optimize hover message retrieval
const hoverMessageCache = new Map();
...
function getHoverMessage(word) {
    ...
    // Check cache first
    if (hoverMessageCache.has(word)) {
        return hoverMessageCache.get(word);
    }
    
    let message;
        if (localFunctions.has(word)) {
            message = `${word}:    ${localFunctions.get(word).content}`;
        } else if (word.startsWith('$') && word !== '$_') {
            message = `${word} is a variable.`;
        } else if (operators[word]) {
            message = operators[word];
        } else {
            message = hoverMessages[word] || `Atom or Symbol`;
        }

        hoverMessageCache.set(word, message);
        return message;
}
...
function logPerformance() {
    const averageTime = performanceMetrics.hoverCount ? 
        performanceMetrics.totalTime / performanceMetrics.hoverCount : 0;
    const hitRate = performanceMetrics.hoverCount ? 
        (performanceMetrics.cacheHits / performanceMetrics.hoverCount) * 100 : 0;
    
    console.log(`Hover Provider Performance:
        Total Hovers: ${performanceMetrics.hoverCount}
